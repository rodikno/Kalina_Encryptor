<?php
function show($x,$title=""){
    $ret="";
    for($j=0;$j<2;$j++){
        for($i=0;$i<8;$i++)
        {
            if($x[$j][$i]<0x10)
                $ret .= "0";
            $ret .= strtoupper(dechex($x[$j][$i]));
        }
        $ret .= ' ';
    }
    if($title!=="")
        echo $title," ";
    echo $ret,'<br/>';
}

function encrypt128($T,$K){
    $K_round = makeRoundKeys128_128($K);
    $G = eta($T,$K_round[0]);
    for($i=1;$i<10;$i++)
        $G = kappa($K_round[$i],psi(tau(pee($G))));
    $G = eta($K_round[10],psi(tau(pee($G))));
    return $G;
}

function decrypt128($C,$K){
    $K_round = makeRoundKeys128_128($K);
    $G = _1_pee(_1_tau(_1_psi(_1_eta($C,$K_round[10]))));
    for($i=9;$i>0;$i--)
        $G = _1_pee(_1_tau(_1_psi(kappa($K_round[$i],$G))));
    $G = _1_eta($G,$K_round[0]);
    return $G;
}

function makeRoundKeys128_128($K){
    $K_s = array(
        array(0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00),
        array(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
    );
    // Intermediate key  
    $KT = psi(tau(pee(eta($K,psi(tau(pee(kappa($K,psi(tau(pee(eta($K_s,$K))))))))))));

    //round keys
    $K_r = array();
    for($i=0;$i<=10;$i+=2)//even
        $K_r[$i] =  eta(fi($KT,8+$i/2),psi(tau(pee(kappa(fi($KT,8+$i/2),psi(tau(pee(eta(fi($KT,8+$i/2),ror($K,(32*$i)%128)))))))))); 
    for($i=1;$i<=10;$i+=2)//odd
        $K_r[$i] = rol($K_r[$i-1],128/4+24);
    return $K_r;
}
// partial functions

function fi($x,$ii){
    $teta = array(
        array(0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01),
        array(0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01)
    );
    return eta(shl($teta,$ii),$x);
}

function shl($x,$ii=1){
    $c = 2;
    //serialize
    $y = array();
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $y[$j*8+$i] = $x[$j][$i];
    
    for($k=0;$k<$ii;$k++){
        $carry_over = 0;
        for($i=$c*8-1;$i>=0;$i--){
            if($y[$i]>=0x80)
                $co = 1;
            else 
                $co = 0;
            $y[$i] = (($y[$i]<<1) | $carry_over) & 0xFF;
            $carry_over = $co;
        }
    }
    //deserialize
    $ret = array(array(),array());
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $ret[$j][$i] = $y[$j*8+$i];
    
    return $ret;
}

function ror($x,$ii=1){
    $c = 2;
    //serialize
    $y = array();
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $y[$j*8+$i] = $x[$j][$i];
    
    for($k=0;$k<$ii;$k++){
        $carry_over = $y[$c*8-1] & 1;
        for($i=0;$i<$c*8;$i++){
            $co = $y[$i] & 1;
            $y[$i] = (($y[$i] >> 1) | ($carry_over << 7)) & 0xFF;
            $carry_over = $co;
        }
    }
    //deserialize
    $ret = array(array(),array());
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $ret[$j][$i] = $y[$j*8+$i];
    
    return $ret;
}

function rol($x,$ii=1){
    $c = 2;
    //serialize
    $y = array();
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $y[$j*8+$i] = $x[$j][$i];
    
    for($k=0;$k<$ii;$k++){
        $carry_over = ($y[0] < 0x80)?0:1;
        for($i=$c*8-1;$i>=0;$i--){
            $co = ($y[$i] < 0x80)?0:1;
            $y[$i] = (($y[$i] << 1) | $carry_over ) & 0xFF;
            $carry_over = $co;
        }
    }
    //deserialize
    $ret = array(array(),array());
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $ret[$j][$i] = $y[$j*8+$i];
    
    return $ret;
}

function kappa($x,$y){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            $ret[$j][$i] = $x[$j][$i]^$y[$j][$i];
    return $ret;
}

function gmul($a,$b) //thanks to http://forum.codenet.ru/q48953/%D0%9E%D0%BF%D0%B5%D1%80%D0%B0%D1%86%D0%B8%D1%8F+%D1%83%D0%BC%D0%BD%D0%BE%D0%B6%D0%B5%D0%BD%D0%B8%D0%B5+%D0%B2+%D0%BF%D0%BE%D0%BB%D0%B5+%D0%93%D0%B0%D0%BB%D1%83%D0%B0+GF(256)
{
    $p = 0;
    for($k = 0; $k < 8; $k++) {
        if(($b & 1) == 1) 
            $p ^= $a;
        $hi_bit_set = ($a & 0x80);
        $a = ($a<<1)&0xFF;
        if($hi_bit_set == 0x80) 
            $a ^= 0x11D; /* x^8 + x^4 + x^3 + x^2 + 1 */
        $b >>= 1;
    }
    return $p;
}

function psi($x){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    
    $v = array(0x01, 0x01, 0x05, 0x01, 0x08, 0x06, 0x07, 0x04);
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
        {
            $acc = 0;
            for($k=0;$k<8;$k++)
                $acc ^= gmul($v[($k-$i+8)%8],$x[$j][$k]);
            $ret[$j][$i] = $acc & 0xFF;
        }
    return $ret;
}

function _1_psi($x){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    
    $v = array(0xAD,0x95,0x76,0xA8,0x2F,0x49,0xD7,0xCA); 
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
        {
            $acc = 0;
            for($k=0;$k<8;$k++)
                $acc ^= gmul($v[($k-$i+8)%8],$x[$j][$k]);
            $ret[$j][$i] = $acc & 0xFF;
        }
    return $ret;
}


function tau($x){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
        {
            $shift = intval(floor($i*128/512));
            /*$val = $x[$j][$i];
            for($k=0;$k<$shift;$k++){
                $right_bit = ($val & 1);
                $val = ($val>>1)|($right_bit<<7);
                
            }*/
            $ret[($j+$shift)%$c][$i] = $x[$j][$i];
        }
    return $ret;
}

function _1_tau($x){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
        {
            $shift = intval(floor($i*128/512));
            $ret[($j-$shift+$c)%$c][$i] = $x[$j][$i];
        }
    return $ret;
}



function eta($x1,$x2){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    
    for($j=0;$j<$c;$j++){
        $carry_over=0;
        for($i=0;$i<8;$i++){//unusual carry-out...  cite: In the addition operation the little-endian convention is used, i.e. less significant bytes have smaller indexes.
            $ret[$j][$i] = $x1[$j][$i] + $x2[$j][$i] + $carry_over;
            if($ret[$j][$i] > 0xFF){
                $ret[$j][$i] &= 0xFF;
                $carry_over = 1;
            }else $carry_over = 0;
        }
    }
    return $ret;
}

function _1_eta($x1,$x2){//x1-x2
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    
    for($j=0;$j<$c;$j++){
        $carry_over=0;
        for($i=0;$i<8;$i++){
            $ret[$j][$i] = $x1[$j][$i] - $x2[$j][$i] - $carry_over;
            if($ret[$j][$i] < 0){
                $ret[$j][$i] = ($ret[$j][$i]+0x100) &0xFF;
                $carry_over = 1;
            }else $carry_over = 0;
        }
    }
    return $ret;
}

function pee($x){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            switch($i%4){
                case 0: $ret[$j][$i] = pi_0($x[$j][$i]);break;
                case 1: $ret[$j][$i] = pi_1($x[$j][$i]);break;
                case 2: $ret[$j][$i] = pi_2($x[$j][$i]);break;
                case 3: $ret[$j][$i] = pi_3($x[$j][$i]);break;
            }
    return $ret;
}

function _1_pee($x){
    $c = 2;
    $ret = array();
    for($i=0;$i<$c;$i++)
        $ret[$i] = array(0,0,0,0,0,0,0,0);
    
    for($j=0;$j<$c;$j++)
        for($i=0;$i<8;$i++)
            switch($i%4){
                case 0: $ret[$j][$i] = _1_pi_0($x[$j][$i]);break;
                case 1: $ret[$j][$i] = _1_pi_1($x[$j][$i]);break;
                case 2: $ret[$j][$i] = _1_pi_2($x[$j][$i]);break;
                case 3: $ret[$j][$i] = _1_pi_3($x[$j][$i]);break;
            }
    return $ret;
}



function pi_0($x){
return array(
)
[$x];
}

function pi_1($x){
return array(
)
[$x];
}

function pi_2($x){
return array(
)
[$x];
}

function pi_3($x){
return array(
)
[$x];
}

function _1_pi_0($x){
return array(
)
[$x];
}

function _1_pi_1($x){
return array(
)
[$x];
}

function _1_pi_2($x){
return array(
)
[$x];
}

function _1_pi_3($x){
return array(
)
[$x];
}